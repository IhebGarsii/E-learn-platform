...
function StudentsAlsoBought({ category }: StudentsAlsoBoughtProps) {
  ...
  console.log(recemondedCourses, "recemondedCourses");

  return (
    <div>
      {recemondedCourses?.map((course: RecemondedCourses) => (
          ...
          <div className="flex aline-center gap-3">
            ...
            <div className="sm:flex sm:align-center sm:justify-between bg-red-100">
              ...
              <div className="flex items-center gap-2 text-gray-500 mt-2">
                <span className="text-yellow-500">{course.courseId.avgRate.nbRate}</span>
                <Rating
                  className="text-xs"
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <span className="text-gray-500">
                  {course.courseId.difficultyLevel} {course.courseId.timeAccess}
                </span>
              </div>
              <h1 className="text-2xl font-bold mt-2">${course.courseId.price}</h1>
            </div>
          </div>
        </div>
      ))}
